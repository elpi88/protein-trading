...
with col_b3:
    if sel_id:
        try:
            from lib.pdf_generator import generate_bid_pdf
            row_data = df[df["Bid ID"].astype(str) == str(sel_id)]
            if not row_data.empty:
                pdf_bytes = generate_bid_pdf(row_data.iloc[0].to_dict())
                st.download_button(
                    "📄 PDF", data=pdf_bytes,
                    file_name=f"bid_{sel_id}.pdf",
